        ...
        <main className="flex-1 p-6 sm:ml-[260px]">
          ...
          <div className="mt-4 text-center">
            <div className="text-xl">
              Berdasarkan gejala yang dipilih, Tanaman Padi anda kemungkinan
              terkena penyakit
            </div>
            <h1 className="text-3xl font-semibold mt-2">
              Nama Penyakit:{" "}
              {diagnoseData?.mostLikelyDisease ?? "Penyakit Tidak Ditemukan"}
            </h1>
            <p className="text-xl mt-2">
              Dengan nilai kepercayan tertinggi yaitu{" "}
              {typeof diagnoseData?.highestProbability === "number"
                ? `${(diagnoseData.highestProbability * 100).toFixed(2)}%`
                : "0%"}
            </p>
            <p>Nilai kepercayaan penyakit lainnya:</p>
            <ul className="list-disc pl-6">
              {Object.entries(diagnoseData?.allProbabilities ?? {}).map(
                ([disease, probability]) => (
                  <li key={disease}>
                    {disease}:{" "}
