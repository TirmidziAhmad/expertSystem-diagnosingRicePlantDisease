...
const TableDiagnosa = () => {
  ...
  const rows = items.map((item) => (
    <Table.Row key={item.name} data-selected={selection.includes(item.name) ? '' : undefined} className='border-b border-gray-300 hover:bg-gray-100 transition-all'>
      <Table.Cell className='py-2 px-4'>
        <Checkbox
          aria-label='Select row'
          checked={selection.includes(item.name)}
          onCheckedChange={(changes) => {
            setSelection((prev) => (changes.checked ? [...prev, item.name] : selection.filter((name) => name !== item.name)));
          }}
        />
      </Table.Cell>
      <Table.Cell className='items-center justify-center'>{item.name}</Table.Cell>
      <Table.Cell className='items-center justify-center'>{item.category}</Table.Cell>
  ...
  return (
    <>
      <Table.Root className='mt-4 border border-gray-300 rounded-md w-full ' striped>
        <Table.Header className='bg-beige text-white'>
          <Table.Row>
            <Table.ColumnHeader className='py-4 px-4'></Table.ColumnHeader>
